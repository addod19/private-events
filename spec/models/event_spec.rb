...
RSpec.describe Event, type: :model do
  context 'event full validations test' do
  ...
  context 'event valid validations test' do
    it 'is invalid without event title' do
      event = Event.new(title: '', location: 'Accra', user_id: '2', description: "Daniel's party with Jack Daniels",
                        date: '2020-02-03')
      event.valid?
      expect(event.errors[:title]).to include("can't be blank")
    end
    it 'is invalid without event location' do
      event = Event.new(title: 'Birthday party', location: '', user_id: '2',
                        description: "Daniel's party with Jack Daniels", date: '2020-02-03')
      event.valid?
      expect(event.errors[:location]).to include("can't be blank")
    end
    it 'is invalid without event description' do
      event = Event.new(title: 'Birthday party', location: '', user_id: '2',
                        description: '', date: '2020-02-03')
      event.valid?
  ...
  context 'event invalid validations test' do
    it 'is invalid with description longer than specified' do
      event = Event.new(title: 'party' * 10, location: '', user_id: '2',
                        description: 'abdcefghjklmpnop' * 20, date: '2020-02-03')
      event.valid?
    ...
    it 'is invalid with description shorter than specified' do
      event = Event.new(title: 'party' * 10, location: '', user_id: '2',
                        description: 'abdc', date: '2020-02-03')
      event.valid?
    ...
    it 'is invalid with title shorter than specified' do
      event = Event.new(title: 'party', location: '', user_id: '2',
                        description: '', date: '2020-02-03')
      event.valid?
